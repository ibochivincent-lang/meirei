...
interface MaskRevealProps {
  /** The heading/line text. Split into words for the staggered reveal. */
  text: string;
 ...
 * MaskReveal
 *
 * Words rise from behind a clip mask, one after another. The site's signature
 * heading treatment — used on the hero and every section title so the page
 * reads as one choreographed piece.
 */
export function MaskReveal({
  ...
  // Find which words fall within the accent substring.
  let accentStart = -1;
  let accentEnd = -1;
  if (accent) {
