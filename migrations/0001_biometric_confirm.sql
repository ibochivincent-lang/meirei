...
create index if not exists tella_webauthn_credentials_user_id_idx
  on public.tella_webauthn_credentials(user_id);
...
create table if not exists public.tella_webauthn_challenges (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.tella_users(id) on delete cascade,
...
create index if not exists tella_webauthn_challenges_user_id_kind_idx
  on public.tella_webauthn_challenges(user_id, kind);

-- 4. Periodic cleanup hint: expired challenges and rows older than a day
--    can be hard-deleted by a cron / SQL job.
--    delete from public.tella_webauthn_challenges where expires_at < now();
