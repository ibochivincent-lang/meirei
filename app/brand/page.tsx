      ...
      <main className="min-h-screen bg-surface-50 text-ink-900">
        <div className="border-b border-ink-200/60 pt-28 pb-14 px-6 sm:px-12 md:pt-36 md:pb-16 bg-[#F6F5EE]">
          <div className="mx-auto max-w-[1400px]">
            <span className="font-mono text-xs font-bold uppercase tracking-widest text-accent-700">
        ...
        <div className="mx-auto max-w-4xl px-6 py-14 sm:px-12 space-y-12">
          ...
          <section className="space-y-4 rounded-2xl border border-ink-200 bg-white p-6 sm:p-8 shadow-xs">
            <h2 className="font-display text-2xl font-bold text-ink-950">Primary Brandmark</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-center justify-between rounded-xl border border-ink-200 bg-surface-50 p-8 text-ink-900 mt-2">
              <div className="scale-110 origin-left">
                <BrandMark />
              </div>
              <span className="font-mono text-xs text-ink-500">Dark on Light / Primary SVG</span>
            </div>
          </section>

          {/* Color Palette */}
          <section className="space-y-4 rounded-2xl border border-ink-200 bg-white p-6 sm:p-8 shadow-xs">
            <h2 className="font-display text-2xl font-bold text-ink-950">Color Architecture</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4 font-mono text-xs">
              <div className="space-y-2">
                <div className="h-20 rounded-xl border border-ink-200 shadow-xs" style={{ backgroundColor: "#F6F5EE" }} />
                <p className="font-bold text-ink-900">Parchment</p>
                <p className="text-ink-500">#F6F5EE</p>
              </div>
              <div className="space-y-2">
                <div className="h-20 rounded-xl bg-ink-950 shadow-xs" />
                <p className="font-bold text-ink-900">Carbon Ink</p>
                <p className="text-ink-500">#0B0B0C</p>
              </div>
              <div className="space-y-2">
                <div className="h-20 rounded-xl bg-[#C4820A] shadow-xs" />
                <p className="font-bold text-ink-900">Research Amber</p>
                <p className="text-ink-500">#C4820A</p>
              </div>
              <div className="space-y-2">
                <div className="h-20 rounded-xl bg-emerald-600 shadow-xs" />
                <p className="font-bold text-ink-900">Settlement Green</p>
                <p className="text-ink-500">#059669</p>
              </div>
            </div>
          </section>
          ...
          {/* Typography */}
          <section className="space-y-4 rounded-2xl border border-ink-200 bg-white p-6 sm:p-8 shadow-xs">
            <h2 className="font-display text-2xl font-bold text-ink-950">Typographic Hierarchy</h2>
            <div className="space-y-4 text-sm mt-4">
              <div className="border-b border-ink-100 pb-3">
                <span className="font-mono text-xs text-ink-400 uppercase">Display Headings</span>
                <p className="font-display text-2xl font-medium text-ink-950 mt-1">Syne / Instrument Serif</p>
              </div>
              <div className="border-b border-ink-100 pb-3">
                <span className="font-mono text-xs text-ink-400 uppercase">Body & Interfaces</span>
                <p className="font-sans text-base text-ink-900 mt-1">Inter / SF Pro Display</p>
              </div>
              <div>
                <span className="font-mono text-xs text-ink-400 uppercase">Monospace & Numerical Telemetry</span>
                <p className="font-mono text-base font-bold text-ink-900 mt-1">JetBrains Mono</p>
              </div>
            </div>
          </section>
