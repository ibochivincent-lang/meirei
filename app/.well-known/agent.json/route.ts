...
export async function GET() {
  const agentManifest = {
    schema_version: "v1",
    name_for_model: "meirei_investment_agent",
    name_for_human: "Meirei Investment Mandate Agent",
    description_for_model:
      "Autonomous non-custodial investment mandate agent for tokenized equities (xStocks) on OKX X Layer (Chain ID 196). Provides real-time USDG spot pricing, autonomous portfolio drift rebalancing, dollar-cost averaging (DCA) schedules, emergency circuit breaker halts, and intent-to-sign calldata generation via OKX Exchange OS.",
    description_for_human:
      "AI-native investment mandate agent for tokenized equities on OKX X Layer.",
    auth: {
      type: "none",
    },
    api: {
    ...
    tools: [
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            amount_usdg: {
              type: "number",
              description: "Allocation in USDG to compute share units for",
            },
          },
          required: ["ticker"],
        },
        ...
        parameters: {
          ...
          properties: {
            ...
            target_weights: {
              type: "object",
              description:
                "Asset tickers mapped to target percentage weights (e.g. {\"NVDAx\": 60, \"AAPLx\": 40})",
            },
            threshold_pct: {
              type: "number",
              description:
                "Drift threshold percentage trigger (default: 5.0%)",
            },
            schedule: {
              type: "string",
              description:
                "Recurrence interval for DCA mandates (e.g. 'weekly_monday_0800_utc')",
            },
            max_drawdown_pct: {
              type: "number",
              description:
                "Drawdown threshold percentage for circuit breaker halt",
            },
          },
          required: ["strategy"],
        },
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            trades: {
              type: "array",
              description: "List of trade diffs needed to realign portfolio",
              items: {
                type: "object",
                properties: {
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            max_drawdown_pct: {
              type: "number",
              description: "Maximum allowable drawdown percentage before halt",
            },
          },
